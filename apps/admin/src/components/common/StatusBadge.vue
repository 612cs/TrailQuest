...
const meta = computed(() => {
  switch (props.status) {
    case 'pending':
      return { label: '待审核', className: 'admin-badge admin-badge-pending' }
    case 'approved':
      return { label: '已通过', className: 'admin-badge admin-badge-approved' }
    case 'rejected':
      return { label: '已驳回', className: 'admin-badge admin-badge-rejected' }
    case 'resolved':
      return { label: '已处理', className: 'admin-badge admin-badge-approved' }
    case 'success':
      return { label: '成功', className: 'admin-badge admin-badge-approved' }
    case 'failed':
      return { label: '失败', className: 'admin-badge admin-badge-rejected' }
    case 'hidden':
