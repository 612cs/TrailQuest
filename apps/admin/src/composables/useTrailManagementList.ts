...
import { fetchAdminTrailManagement } from '../api/admin'
import type { AdminTrailListItem } from '../types/admin'

export function useTrailManagementList() {
  ...
  async function load(page = pageNum.value) {
    ...
    errorMessage.value = ''
    try {
      const result = await fetchAdminTrailManagement({
        pageNum: page,
      ...
      errorMessage.value = error instanceof Error ? error.message : '路线管理列表加载失败'
    } finally {
      loading.value = false
    }
  }
