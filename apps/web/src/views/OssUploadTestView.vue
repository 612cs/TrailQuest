...
<template>
  <main class="min-h-screen">
    ...
    <div class="mx-auto max-w-3xl space-y-5 px-4 py-6 sm:px-6">
      <section class="card animate-fade-in-up space-y-4 p-5">
        <div>
          <h1 class="text-xl font-bold" style="color: var(--text-primary)">最小上传验证</h1>
          <p class="mt-1 text-sm" style="color: var(--text-secondary)">
            这页会真实调用 STS、直传 OSS，并在上传完成后调用 `/api/uploads/complete` 保存
            `media_files` 记录。
          </p>
        </div>
        ...
        <div class="space-y-3">
          <label
            class="hover:border-primary-500 hover:bg-primary-500/5 flex min-h-48 cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed px-6 py-8 text-center transition-colors"
          ...
          <div
            ...
            style="border-color: var(--border-default)"
          >
            <img :src="localPreviewUrl" alt="本地预览" class="h-72 w-full object-cover" />
          </div>
        </div>
        ...
        <div v-if="isUploading" class="space-y-2">
          ...
          <div class="h-2 overflow-hidden rounded-full" style="background-color: var(--bg-page)">
            <div
      ...
      <section v-if="uploadResult" class="card animate-fade-in-up space-y-3 p-5">
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          ...
          >
            ...
            <img
              :src="uploadResult.localPreviewUrl"
              alt="本地预览"
          ...
          >
            ...
            <p class="text-sm break-all" style="color: var(--text-primary)">
              mediaId: {{ uploadResult.remote.mediaId }}
            </p>
            <p class="text-sm break-all" style="color: var(--text-primary)">
              bizType: {{ uploadResult.remote.bizType }}
            </p>
            <p class="text-sm break-all" style="color: var(--text-primary)">
              objectKey: {{ uploadResult.remote.objectKey }}
            </p>
            <a
              :href="uploadResult.remote.url"
              target="_blank"
              rel="noreferrer"
              class="text-primary-500 inline-flex items-center gap-1 text-sm font-medium"
            >
              打开 OSS 文件
