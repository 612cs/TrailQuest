...
<template>
  <main class="min-h-[100vh] p-0">
    <div class="mx-auto flex h-[100vh] max-w-7xl gap-0 p-0 lg:gap-4 lg:p-4">
      ...
      <section
        class="flex min-w-0 flex-1 flex-col overflow-hidden rounded-[32px] border"
        style="border-color: var(--border-card); background-color: var(--bg-card)"
      >
        <header
        ...
        >
          <div class="flex items-center gap-3">
            <button
              @click="router.push('/')"
              class="hover:bg-primary-500/5 flex h-10 w-10 items-center justify-center rounded-2xl border transition-colors"
            ...
            <div>
              <p class="text-primary-500 text-xs tracking-[0.24em] uppercase">TrailQuest AI</p>
              <h1 class="mt-1 text-lg font-semibold sm:text-xl" style="color: var(--text-primary)">
                路线顾问
              </h1>
            </div>
          </div>
          ...
          <div
            class="hidden items-center gap-2 rounded-full border px-4 py-2 text-xs sm:flex"
        ...
        <div ref="messagesContainer" class="flex-1 overflow-y-auto px-4 py-5 sm:px-6">
          ...
          >
            <div
              class="flex h-16 w-16 items-center justify-center rounded-[24px] border"
              style="
                border-color: var(--border-default);
                background-color: color-mix(in srgb, var(--color-primary-500) 8%, var(--bg-card));
                color: var(--color-primary-500);
              "
            >
              <BaseIcon name="Leaf" :size="28" />
            </div>
            <h2 class="mt-5 text-xl font-semibold" style="color: var(--text-primary)">
              登录后开启 TrailQuest AI
            </h2>
            ...
            <button
              class="hover:bg-primary-500/5 mt-6 rounded-full border px-5 py-2.5 text-sm font-medium transition-colors"
              style="border-color: var(--border-default); color: var(--text-primary)"
          ...
          >
            ...
            <div class="mt-8 grid w-full max-w-3xl grid-cols-1 gap-3 md:grid-cols-2">
              <button
                ...
                :key="item.text"
                class="hover:bg-primary-500/5 rounded-[24px] border px-4 py-4 text-left transition-all duration-200 hover:-translate-y-0.5"
              ...
              >
                <div class="flex items-start gap-3">
                  <div
                    ...
                    style="border-color: var(--border-default); color: var(--color-primary-500)"
                  >
                    <BaseIcon :name="item.icon" :size="18" />
                  </div>
                  <p class="text-sm leading-7" style="color: var(--text-primary)">
                    {{ item.text }}
                  </p>
                </div>
              </button>
    ...
    <transition name="fade">
      ...
      >
        <aside class="absolute top-0 left-0 h-full w-[86vw] max-w-[360px] p-3" @click.stop>
          <ChatSidebar
            :conversations="chatStore.conversations"
            :active-conversation-id="chatStore.activeConversationId"
            :loading="chatStore.isBootstrapping"
