...
function extractPaths(geoJson: unknown): TrackViewerPath[] {
  ...
  for (const feature of features) {
    ...
    const pathName =
      typeof typedFeature.properties?.name === 'string' ? typedFeature.properties.name : undefined
    const pathColor =
      typeof typedFeature.properties?.stroke === 'string'
        ? typedFeature.properties.stroke
        : undefined
...
function resolveDistanceMeters(paths: TrackViewerPath[], explicitDistanceMeters?: number | null) {
  if (
    typeof explicitDistanceMeters === 'number' &&
    Number.isFinite(explicitDistanceMeters) &&
