...
const isUserNotFoundError = computed(
  () => currentView.value === 'login' && errorCode.value === 'USER_NOT_FOUND',
)
</script>
    ...
    <div class="space-y-6">
      <div v-if="currentView === 'login'" class="space-y-6">
        ...
        <form class="space-y-4" @submit.prevent="handleSubmit">
          <div
            v-if="errorMessage"
            class="mb-4 flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm"
            style="background-color: var(--bg-tag); color: var(--color-hard)"
          >
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="min-w-0 flex-1">
                ...
                <button
                  type="button"
                  class="bg-primary-500 hover:bg-primary-600 mt-3 inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium text-white transition-colors"
          ...
          <div class="space-y-1">
            ...
            <div class="relative">
              ...
              <input
                ...
                class="focus:ring-primary-500/30 w-full rounded-xl border py-2.5 pr-4 pl-10 text-sm transition-all focus:ring-2 focus:outline-none"
                style="
          ...
          <div class="space-y-1">
            <div class="flex items-center justify-between">
              <label class="block text-sm font-medium" style="color: var(--text-primary)"
                >密码</label
              >
              <a href="#" class="text-primary-500 text-xs font-medium hover:underline"
                >忘记密码？</a
              >
            </div>
            <div class="relative">
          ...
          <button
            ...
            :class="
              !canSubmitLogin || isLoading
                ? 'opacity-70'
                : 'bg-primary-500 hover:bg-primary-600 hover:shadow-primary-500/30 hover:shadow-lg active:scale-[0.98]'
            "
            :style="
              !canSubmitLogin || isLoading ? 'background-color: var(--color-surface-400);' : ''
            "
          >
        ...
        <div
          class="border-t pt-4 text-center text-sm"
          style="border-color: var(--border-default); color: var(--text-secondary)"
        >
          还没有账户？
          <button
            class="text-primary-500 font-semibold transition-colors hover:underline"
            @click="goToRegisterProfile"
          >
            立即注册
          </button>
        </div>
      ...
      <div v-else-if="currentView === 'register-profile'" class="space-y-6">
        <div class="space-y-2">
          <p class="text-sm" style="color: var(--text-secondary)">
            这一步是可选的。告诉我们你的徒步偏好，后续推荐会更贴近你，也
            <b class="text-primary-500 text-base">可以先跳过</b> 直接注册。
          </p>
        </div>
        ...
        <div class="flex gap-3">
          <button
            type="button"
            class="hover:border-primary-500/30 hover:bg-primary-500/6 flex-1 rounded-xl border px-4 py-3 text-sm font-medium transition-colors"
            style="border-color: var(--border-default); color: var(--text-secondary)"
          ...
          <button
            type="button"
            class="bg-primary-500 hover:bg-primary-600 flex-1 rounded-xl px-4 py-3 text-sm font-medium text-white transition-colors disabled:cursor-not-allowed disabled:opacity-60"
      ...
      <div v-else class="space-y-6">
        ...
        <form class="space-y-4" @submit.prevent="handleSubmit">
          ...
          <div class="flex gap-3 pt-2">
            <button
              type="button"
              class="hover:border-primary-500/30 hover:bg-primary-500/6 rounded-xl border px-4 py-3 text-sm font-medium transition-colors"
              style="border-color: var(--border-default); color: var(--text-secondary)"
              @click="backToProfile"
            >
              上一步
            ...
            <button
              ...
              :class="
                !canSubmitRegister || isLoading
                  ? 'opacity-70'
                  : 'bg-primary-500 hover:bg-primary-600 hover:shadow-primary-500/30 hover:shadow-lg active:scale-[0.98]'
              "
              :style="
                !canSubmitRegister || isLoading ? 'background-color: var(--color-surface-400);' : ''
              "
            >
