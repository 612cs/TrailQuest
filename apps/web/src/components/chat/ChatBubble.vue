...
<template>
  <div
    class="animate-fade-in-up flex max-w-[92%] gap-3"
  ...
  >
    ...
    >
      <template v-if="message.role === 'user'">
        ...
        />
        <span v-else class="text-sm leading-none font-semibold">{{
          props.userAvatarText || 'U'
        }}</span>
      </template>
    ...
    <div class="min-w-0 space-y-2">
      ...
      >
        <p class="break-words whitespace-pre-wrap">
          {{ message.content || (message.isStreaming ? '正在整理路线建议...' : '') }}
        </p>
        <span
        ...
        >
          <button
            ...
            class="hover:bg-primary-500/5 rounded-full border px-3 py-1.5 text-xs transition-colors sm:text-sm"
