...
const activities = computed(() =>
  optionConfigStore
    .getGroup('home_activity', [...DEFAULT_ACTIVITIES])
...
<template>
  <section class="py-10 sm:py-16" style="background-color: var(--bg-card)">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
      <h2 class="mb-6 text-xl font-bold sm:mb-8 sm:text-2xl" style="color: var(--text-primary)">
        按活动探索
      </h2>
      <div class="grid grid-cols-3 gap-3 sm:grid-cols-6 sm:gap-4">
        <button
          ...
          :key="activity.code"
          class="card card-hover animate-fade-in-up flex flex-col items-center justify-center gap-2 px-3 py-5 sm:py-6"
          :class="`stagger-${i + 1}`"
        ...
        >
          ...
          <span class="text-xs font-medium sm:text-sm" style="color: var(--text-primary)">{{
            activity.name
          }}</span>
        </button>
      </div>
