...
<template>
  <transition name="flash-toast">
    <div
      ...
      class="fixed top-5 left-1/2 z-[70] flex max-w-[calc(100vw-2rem)] min-w-[220px] -translate-x-1/2 items-center gap-2 rounded-2xl border px-4 py-3 shadow-lg backdrop-blur-md"
      :style="
        flashStore.message.type === 'success'
    ...
    >
      <BaseIcon
        ...
        :size="18"
        :class="
          flashStore.message.type === 'success' ? 'text-primary-500' : 'text-[var(--color-hard)]'
        "
      />
      <span class="text-sm leading-5 font-medium">{{ flashStore.message.message }}</span>
    </div>
  </transition>
</template>
