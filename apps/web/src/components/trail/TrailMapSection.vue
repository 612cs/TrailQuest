    ...
    <div class="card space-y-4 p-4">
      <div class="map-shell" :class="{ 'is-empty': (!hasCenter && !hasTrack) || !mapReady }">
        ...
        <div
          ref="map"
          class="map-canvas"
          :style="{ display: mapReady && (hasCenter || hasTrack) && !mapError ? 'block' : 'none' }"
        ></div>
        <div v-if="!hasCenter && !hasTrack" class="map-placeholder">暂无法获取定位，稍后重试</div>
        <div v-else-if="mapError" class="map-placeholder">
          {{ mapError }}
        </div>
        <div v-else-if="!mapReady" class="map-placeholder z-10">地图加载中...</div>
      </div>
    </div>
  </section>
</template>
