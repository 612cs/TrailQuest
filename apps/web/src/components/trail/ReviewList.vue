  ...
  <section class="animate-fade-in-up stagger-4">
    <SectionHeader title="用户评价">
      <template #action>
        <div class="flex items-center gap-1">
          ...
          <span class="text-primary-500 text-sm font-bold">{{ averageRating ?? 0 }}</span>
          <span class="text-xs" style="color: var(--text-tertiary)"
            >({{ effectiveTotalReviews }} 条)</span
          >
        </div>
      </template>
    </SectionHeader>
    ...
    <div v-else class="space-y-3">
      ...
      <div v-if="loadMoreError" class="card p-4 text-center">
        ...
        <button
          type="button"
          class="hover:bg-primary-500/10 mt-3 inline-flex items-center gap-1 rounded-full px-4 py-2 text-sm font-medium transition-colors"
